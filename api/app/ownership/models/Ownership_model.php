...
class Ownership_model extends CI_Model
{
    public function __construct()
    {
		...
		$this->max_data = 100;
		$this->table = 'users_ms_ownership_types';
    }
	...
	public function insert($param,$token)
	{
		try {
			...
			foreach($param as $row){
				$data = array();
				$data['ownership_type_code'] = isset($row['ownership_type_code']) ? $row['ownership_type_code'] : "";
				$data['ownership_type_name'] = isset($row['ownership_type_name']) ? $row['ownership_type_name'] : "";
				$data['status'] = isset($row['status']);

				$unique_field = array(
										'from_table' => $this->table, 
										'unique_field' => 'ownership_type_code' 
									);
