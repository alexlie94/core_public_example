$(document).ready(function () {
  ...
  var channelBody =
    '<tr id="id_{{sourceID}}_{{channelID}}" data-source="{{sourceID}}" data-channel="{{channelID}}">' +
    "<td>{{no}}</td>" +
    ...
    `<td>
      <button class="btn btn-outline btn-outline-dashed btn-outline-success btn-sm me-2 btnImageNotDefault" data-source="{{sourceID}}" data-channel="{{channelID}}" data-fullscreenmodal="1" data-type="modal" 
      data-url="` +
    base_url() +
    `inventory_display/notdefault/{{sourceID}}/{{channelID}}/{{productID}}" {{disabledButton}}>Image</button>
      <button class="btn btn-outline btn-outline-dashed btn-outline-{{colorStatus}} btn-sm me-2 btnActionNotDefault btnActionNotDefault{{sourceID}}_{{channelID}}" data-source="{{sourceID}}" data-channel="{{channelID}}" data-status="{{status}}" data-color="btn-outline-{{colorStatus}}" data-type="modal" 
      data-url="` +
    base_url() +
    `inventory_display/launch/{{sourceID}}/{{channelID}}/{{productID}}" {{disabledButton}}>{{textButtonAction}}</button>
      <button id="action_publish" class="btn btn-outline btn-outline-dashed btn-outline-warning btn-sm me-2" data-type="modal" 
      data-url="{{url_publish}}" {{disabledButtonPublish}}>Publish</button>
    </td>` +
  ...
  $(document).on("click", "#addSource", function () {
    ...
    request(data, btn, function (response) {
      loadingButtonOff(
        btn,
        '<i class="bi bi-plus-lg fs-4 me-2"></i>' + btnText
      );
  ...
  function validate() {
    ...
    $(".validateSource").each(function () {
      ...
      var value = $(this).val();
      if (value == "" || value == null) {
        var messageData = errorMessage.replaceAll(
          "{{message}}",
  ...
  function addRow(
    ...
    colorStatus = "primary",
    status = 4,
    textButtonAction = "Launch",
    textStatus = "Image Not Selected, Pending"
  ) {
    var no = 1;
    ...
    let checkButtonStatus = textButtonAction;
    let disabledButtonPublish = "";
    if (checkButtonStatus === "Pending") {
      disabledButtonPublish = "";
    } else {
      disabledButtonPublish = "disabled";
    }

    let build_array = {
      mode: "add",
      type: "inv_display",
      source: sourceNotDefault,
      channel: channelNotDefault,
      product_id: productID,
      form_type: "single",
    };

    const params = btoa(JSON.stringify(build_array));
    ...
    request(sendData, btn, function (response) {
      ...
      if (response.success) {
        if (typeof response.launch != "undefined") {
          ...
          $(".btnActionNotDefault" + source + "_" + channel).text(
            lookupDisplay[4]
          );
          $(".btnActionNotDefault" + source + "_" + channel).attr(
            "data-status",
            4
          );
          var colour = $(".btnActionNotDefault" + source + "_" + channel).attr(
            "data-color"
          );
          $(".btnActionNotDefault" + source + "_" + channel).removeClass(
            colour
          );
          $(".btnActionNotDefault" + source + "_" + channel).addClass(
            "btn-outline-" + lookupDisplayColour[4]
          );
          $(".btnActionNotDefault" + source + "_" + channel).attr(
            "data-color",
            "btn-outline-" + lookupDisplayColour[4]
  ...
  $(document).on("click", ".btnLaunchProductSource", function () {
    ...
    request(data, btn, function (response) {
      ...
      if (response.success) {
        ...
        $(".btnActionNotDefault" + source + "_" + channel).addClass(
          "btn-outline-" + lookupDisplayColour[5]
        );
        $(".btnActionNotDefault" + source + "_" + channel).attr(
          "data-color",
          "btn-outline-" + lookupDisplayColour[5]
