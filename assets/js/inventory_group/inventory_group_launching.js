$(document).ready(function () {
  ...
  var channelBody =
    ...
    '<td><button class="btn btn-outline btn-outline-dashed btn-outline-success btn-sm me-2 btnImageNotDefault" data-source="{{sourceID}}" data-channel="{{channelID}}" data-fullscreenmodal="1" data-type="modal" data-url="' +
    base_url() +
    "inventory_group/notdefault/{{sourceID}}/{{channelID}}/{{productID}}" +
    '" {{disabledButton}}>Image</button><button class="btn btn-outline btn-outline-dashed btn-outline-{{colorStatus}} btn-sm me-2 btnActionNotDefault btnActionNotDefault{{sourceID}}_{{channelID}}" data-source="{{sourceID}}" data-channel="{{channelID}}" data-status="{{status}}" data-color="btn-outline-{{colorStatus}}" data-type="modal" data-url="' +
    base_url() +
    "inventory_group/launch/{{sourceID}}/{{channelID}}/{{productID}}" +
    '" {{disabledButton}}>{{textButtonAction}}</button></td>' +
  ...
  function initRow() {
    dataLaunching.forEach((element) => {
      ...
      var product = element.users_ms_inventory_groups_id;
  ...
  $(document).on("click", "#btnProcessModalDefault", function () {
    ...
    var productid = btn.data("productgid");
    ...
    sendData.push({ name: "productgid", value: productid });
  ...
  $(document).on("click", "#btnProcessModalNotDefault", function () {
    ...
    sendData.push({ name: "source", value: source });
    sendData.push({ name: "channel", value: channel });
    sendData.push({ name: "gid", value: gid });
    request(sendData, btn, function (response) {
      loadingButtonOff(btn, textBtn);
      message(response.success, response.messages);
      if (response.success) {
