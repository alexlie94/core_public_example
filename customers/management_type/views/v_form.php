...
<div class="form-group row mb-8" id="show_mass_upload">
    <div class="col-md-7">
        ...
        <input type="file" id="upload_data" class="form-control mb-3 mb-lg-0 mt-19" accept=".csv" data-type="input" autocomplete="off" />
    ...
    <div class="col-md-5 mt-3">
        <a href="<?= base_url('/assets/excel/List_Category.csv') ?>" download="List_Category.csv" target="_blank" class="text-gray-800 text-hover-primary d-flex flex-column mb-3 text-center">
    ...
    <table id="kt_datatable_vertical_scroll" class="table table-striped table-row-bordered gy-5 gs-7">
        <thead>
            <tr class="fw-semibold fs-6 text-gray-800">
                <th>#</th>
                <th class="min-w-200px">Category Code</th>
                <th class="min-w-200px">Category Name</th>
                <th class="min-w-200px">Parent Category Name</th>
            </tr>
        </thead>
        <tbody>
        </tbody>
...
<div id="form_brand">
    <div class="d-flex flex-column scroll-y me-n7 pe-7">
        ...
        <input type="hidden" class="form-control" id="id" name="id" value="<?= isset($dataItems['id']) ? $dataItems['id'] : '' ?>" />

        <div class="fv-row mb-7">
            <label class="fw-semibold fs-6 mb-4">Category Code</label>
            <input type="text" id="category_code" name="category_code" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Category Code" value="<?= isset($dataItems['categories_code']) ? $dataItems['categories_code'] : '' ?>" data-type="input" autocomplete="off" />
        </div>

        <div class="fv-row mb-7">
            <label class="required fw-semibold fs-6 mb-4">Category Name</label>

            <input type="text" id="category_name" name="category_name" class="form-control form-control-solid mb-3 mb-lg-0" placeholder="Category Name" value="<?= isset($dataItems['categories_name']) ? $dataItems['categories_name'] : '' ?>" data-type="input" autocomplete="off" />
        </div>

        <div class="fv-row mb-7">
            <label class="fw-semibold fs-6 mb-4">Category Parent</label>

            <select data-control="select2" data-type="select-multiple" class="form-select form-select-solid " id="parent_id" name="parent_id">
                ...
                ?>
                    <option value='<?= $res->id ?>' <?= $val ?>><?= $res->categories_name ?></option>
                <?php } ?>
            </select>
        </div>
    </div>
