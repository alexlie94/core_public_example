...
class Inventory_display extends MY_Customers
{
    ...
    {
        $this->_function_except = ['getchannel', 'launchproductsourceshadow' ,'setdefaultimageshadow','notdefaultshadowprocess','defaultprocessshadow' ,'launchshadow','notdefaultshadow' ,'defaultshadow', 'shadowlaunching','processaddshadow','addshadow','show', 'sku', 'shadow','launching','default','defaultprocess','addsource','addchannel','notdefault','confirmimage','viewimage','notdefaultprocess','setdefaultimage','launch','launchproductsource','download','upload','document','downloadtemplatecsv','processupload','checkingdata'];
        parent::__construct();
        $this->_searchBy = [
    ...
    public function launch($source,$channel,$productID)
    {
        ...
        try {
           ...
           $data = 
            ...
            $button .= "<button type=\"button\" class=\"btn btn-primary btnLaunchProductSource\" id=\"btnLaunchedPublish\" data-params=\"{$params}\" data-source = \"{$source}\" data-channel = \"{$channel}\" data-productid= \"{$productID}\" data-url=\"{$buttonUrl}\">Launch</button>";
    ...
    public function upload()
    {
        ...
        $dataShow = [
            'url_form' => base_url() . 'inventory_display/document',
        ];
    ...
    public function document()
    {
        ...
        $response = $this->inventory_display_model->import();
    ...
    private function preview()
    {
        ...
        try {
            ...
            $preview = $this->inventory_display_model->preview();
            ...
            if($statusDataSaving){
                ...
                $buttonUrl  = base_url()."inventory_display/processupload";
            }else{
                $buttonName = 'Check Data';
                $buttonUrl = base_url()."inventory_display/checkingdata";
            }
    ...
    public function processupload()
    {
        ...
        $response = $this->inventory_display_model->checkingData();
        if($response['success']){
            $response = $this->inventory_display_model->saveUpload();
        }
    ...
    /* shadow */
    public function shadow($id = null)
    {
        $this->function_access('update');
        $this->template->title('Product Shadow');
        $this->setTitlePage('Product Shadow');
        $this->assetsBuild(['datatables']);

        $headerTable = ['No','Product ID','Product Name','Status','Shadow Launch Date','Action'];
        $data = $this->inventory_display_model->shadow($id);

        $detailTable = ['no','product ID','SKU','General Color','Variant Color','Size','Status'];
        $this->setTemplateTable($detailTable);

        $showData = [
            'headerTable' => generateTableHtml($headerTable,$data['header'],"tableVariant"),
            'backUrl' => $data['backUrl'],
            'productID' => $id,
            'urlAddShadow' => base_url()."inventory_display/addshadow/{$id}",
        ];

        $this->setJs('inventory_display_shadow');
        $this->template->build('v_shadow', $showData);
    }

    public function sku($productID = null)
    {
        isAjaxRequestWithPost();
		$this->function_access('view');
        if($productID == null){
		...
		echo $this->inventory_display_model->_getInventoryDisplayDatatables()->show($productID);
    }

    public function addshadow($productID = null)
    {   
        isAjaxRequestWithPost();
		$this->function_access('insert');
        ...
        try {
            $data = array(
                'title_modal' => 'Add Shadow',
                'url_form' => base_url() . "inventory_display/processaddshadow/{$productID}",
                'form' => $this->load->view('v_formshadow', null, true),
                'buttonID' => "btnProcessAddShadow",
                'buttonName' => "Add"
            );
    ...
    public function processaddshadow($productID = null)
    {
        isAjaxRequestWithPost();
        $this->function_access('insert');
        $response = $this->inventory_display_model->processAddShadow($productID);
        echo json_encode($response);
    }

    public function shadowlaunching($productIDShadow)
    {
        $this->function_access('update');
        $this->template->title('Product Shadow Launching');
        $this->setTitlePage('Product Shadow Launching');
        $this->assetsBuild(['datatables']);

        $check = $this->inventory_display_model->_getProductShadows()->get(array('users_ms_product_shadows_id' => $productIDShadow));
        if(!$check){
            pageError();
        }

        $data = $this->inventory_display_model->shadowLaunching($productIDShadow);
    
        $headerTable = ["No","ProductID","Product","SKU","General Color","Variant Color"];

        $sourceDefault = ['No','Source', 'Channel', 'Action', 'Status'];
        $defaultImage = $this->inventory_display_model->_getInventoryDisplayDefaultShadow()->get(array('users_ms_product_shadows_id' => $check->id,'image_status' => 3));
        $statusdefaultImage = 'Image Not Selected';
        if(is_object($defaultImage)){
            $statusdefaultImage = 'Image Selected';
        }
        ...
        $statusDefaultImageHtml = "<span id=\"statusDefaultImage\">{$statusdefaultImage}</span>";
        $urlDefaultImage = base_url("inventory_display/defaultshadow/default/default/{$productIDShadow}");
        $buttonDefault = "<button class=\"btn btn-outline btn-outline-dashed btn-outline-success btn-sm\" id=\"btnDefaultImage\" data-source=\"default\" data-channel=\"default\" data-fullscreenmodal=\"1\" data-type=\"modal\"  data-url=\"{$urlDefaultImage}\">Select Default Image</button>";
        $sourceDefaultData[0] = ['No' => '1','Source'=>'Default','Channel' => 'Default','Action' => $buttonDefault, "Status" => $statusDefaultImageHtml];
    
        $sourceTable = ["No","Source","Channel","Launch Date","Action","Status"];

        $dataLaunching = $this->inventory_display_model->showLaunchingShadow($productIDShadow);
        ...
        $this->setJs('inventory_display_launching_shadow');
        $this->template->build('v_launchingshadow', $showData);
    }
    ...
    public function notdefaultshadow($source,$channel,$productID)
    {
        ...
        try {
            ...
            $get = $this->inventory_display_model->_getProductShadows()->get(array('users_ms_product_shadows_id' => $productID));
            if(!$get){
                throw new Exception("Failed request data", 1);
            ...
            $dataShow = $this->inventory_display_model->notDefaultShadow($source,$channel,$productID);
            ...
            $urlSaving = base_url("inventory_display/notdefaultshadowprocess");
            $dataInput = "data-source='{$source}' data-channel='{$channel}' data-productid='{$productID}' data-url='{$urlSaving}'";
            $data = array(
                'title_modal' => "Product Image",
                'content' => $this->load->view('v_notdefaultshadow',$showHtml,true), 
                'dataInput' => $dataInput,
                'buttonID' => 'btnProcessModalNotDefault',
                'buttonDisabled' => $statusButtonSave,
            );
    ...
    public function defaultshadow($source,$channel,$productID)
    {
        isAjaxRequestWithPost();
        $this->function_access('view');
        try {
            ...
            $dataShow = $this->inventory_display_model->defaultShadow($productID);
            if(isset($dataShow['error'])){
                throw new Exception($dataShow['error'], 1);
            ...
            $urlSaving = base_url("inventory_display/defaultprocessshadow");
            $dataInput = "data-source='{$source}' data-channel='{$channel}' data-productid='{$productID}' data-url='{$urlSaving}'";
            $data = array(
                'title_modal' => "Product Shadow Default Image ",
                'content' => $this->load->view('v_default',$showHtml,true), 
                'dataInput' => $dataInput,
                'buttonID' => 'btnProcessModalDefault',
                'buttonDisabled' => $statusButtonSave,
    ...
    public function defaultprocessshadow()
    {
        ...
        $response = $this->inventory_display_model->defaultProcessShadow();
        echo json_encode($response);
    }
    ...
    public function setdefaultimageshadow()
    {
        isAjaxRequestWithPost();
        $this->function_access('update');
        $response = $this->inventory_display_model->setDefaultImageShadow();
        echo json_encode($response);
    }

    public function notdefaultshadowprocess()
    {
        isAjaxRequestWithPost();
        $this->function_access('update');

        $response = $this->inventory_display_model->notDefaultProcessShadow();
        echo json_encode($response);
    }

    public function launchshadow($source,$channel,$productID)
    {
        isAjaxRequestWithPost();
        $this->function_access('update');
        try {
            ...
            $button = "<button type=\"button\" class=\"btn btn-light me-2\" data-bs-dismiss=\"modal\">Close</button>";
            $buttonUrl = base_url("inventory_display/launchproductsourceshadow");
    ...
    public function launchproductsourceshadow()
    {
        isAjaxRequestWithPost();
        $this->function_access('update');
        $response = $this->inventory_display_model->launchProductSourceShadow();
        echo json_encode($response);
    }
}
