...
class Rolepermissions_model extends MY_ModelCustomer
{
    use MY_Tables;
    ...
    public function show($button)
    {
        $this->datatables->select("a.id,
            a.role_name,
            (select count(bb.id) from {$this->_table_users_ms_access} aa inner join {$this->_table_users} bb ON bb.id = aa.{$this->_table_users}_id where aa.{$this->_tabel}_id = a.id and bb.deleted_at is null) as jumlah_user,
            IF(a.status = 1, 'enable','disable') as status,
            ", false);
        // if(!empty($this->input->post('search'))){
        //     $this->datatables->like('a.role_name',$this->input->post('search'));
        // }
        ...
        if ($filters !== false && is_array($filters)) {
            ...
            if (!empty($getSearchBy)) {
                switch ($getSearchBy) {
                    case 'role_name':
                        $this->datatables->like('a.role_name', $setValue);
                        break;
                    default:
                        break;
                }
        ...
        $this->datatables->where(array("{$this->_table_users_ms_companys}_id" => $this->_users_ms_companys_id));
    ...
    public function validationDelete($id)
    {
        ...
        try {
            ...
            if ($users) {
                $cek = $this->_getAccess()->getData(array("{$this->_table_users_ms_roles}_id" => $id, "{$this->_table_users}_id" => $users->id));
                if ($cek) {
                    throw new Exception("Sorry, you don't have permission to access", 1);
                }
            }

            $cekRelasi = $this->_getAccess()->getData(array("{$this->_table_users_ms_roles}_id" => $id));
            if ($cekRelasi) {
    ...
    public function save($class = '')
    {
        ...
        try {
            ...
            if ($class != '') {
                $checkPermission = $this->_getMenu()->getSpecificMenu(array('controller' => $class));
            ...
            foreach ($roleAccess as $ky => $val) {
                ...
                if (empty($id)) {
                    $data_permission["{$this->_table_users_ms_menus}_id"] = $menu_id;
                    $data_permission["{$this->_table_users_ms_roles}_id"] = $process_role;
                    $process = $this->_getRoleAccess()->insert($data_permission);
                } else {
                    $array_search = array("{$this->_table_users_ms_roles}_id" => $id, "{$this->_table_users_ms_menus}_id" => $menu_id);
                    $check = $this->_getRoleAccess()->get($array_search);
                    if ($check) {
                        $process = $this->_getRoleAccess()->update($array_search, $data_permission);
                    } else {
                        $data_permission["{$this->_table_users_ms_menus}_id"] = $menu_id;
                        $data_permission["{$this->_table_users_ms_roles}_id"] = $id;
                        $process = $this->_getRoleAccess()->insert($data_permission);
                    }
                }
            ...
            $response['messages'] = (empty($id) ? 'Insert Data' : 'Update Data') . " & role permission successful.";
            $response['type'] = empty($id) ? 'insert' : 'update';
            if ($permissionOff) {
                $menu_first = $this->_getApp()->validation_menu($this->_session_email, $this->_session_id, $this->_users_ms_companys_id, 'get');
    ...
    public function getMenuUpdate($id)
    {
        ...
        $role = $this->_getRoleAccess()->get_all(array("{$this->_table_users_ms_roles}_id" => $id));

        foreach ($role as $ky => $val) {
            $menu_id = $val->{$this->_table_users_ms_menus . "_id"};
            $view = $val->view;
            $insert = $val->insert;
            $update = $val->update;
            $delete = $val->delete;
    ...
    public function changeStatus($id)
    {
        try {
            ...
            $users = $this->_getUsers()->get(array('remember_token' => $this->_session_id));
            if ($users) {
                $cek = $this->_getAccess()->getData(array("{$this->_table_users}_id" => $users->id, "{$this->_table_users_ms_roles}_id" => $id));
                if ($cek) {
                    throw new Exception("Sorry, you don't have permission to access", 1);
                }
            }
    ...
    public function deleteData($id)
    {
        ...
        try {
            ...
            $deleteRoleAccess = $this->_getRoleAccess()->delete(array("{$this->_table_users_ms_roles}_id" => $id));
            if (!$deleteRoleAccess) {
